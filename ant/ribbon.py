...
def antCGiRibbonNews():
    ...
    cmds.scrollField('antcgiRibbonNews', e=1, it='----------------------------------------------\n', ip=0)
...
def buildIcon(iconName, scale, *args):
    ...
    pointsList = (
     [0.49949352081828996, -0.4996564674195479, 0.00011386882060004933],
     [-0.5005038397237953, -0.4996577748491638, 0.0002594457838510644],
     [-0.5005038886496341, 0.500343238868989, -0.00011386824481751656],
     [0.49950191468729815, 0.5003427836830858, -0.0002594457838509534],
     [0.49949352081828996, -0.4996564674195479, 0.00011386882060004933])
...
def cleanRibbon(ribbonNme, matrixRibbon):
    ...
    for extraNode in extraNodeList:
        if 'Constraint' not in extraNode:
            cmds.parent(extraNode, ribbonNme + '_grp')
...
def convertToMatrix(*args):
    ...
    if not cmds.objExists(ribbonNme + '_follicle_01'):
        cmds.error('Follicles not found, make sure the name of the ribbon is correct.')
...
def addRibbonDeformers(surface, ribbonDir, *args):
    cmds.duplicate(surface, n=(surface + '_sine_bsh'))
    cmds.duplicate(surface, n=(surface + '_twist_bsh'))
...
def useRibbon(*args):
    ...
    if selRibbonList:
        for selRibbon in selRibbonList:
            if not cmds.attributeQuery('jointOrientMenu', ex=1, n=selRibbon):
                cmds.error('Unable to update this ribbon, please use one generated with this tool.')
            else:
                ...
                orientation = (
                 cmds.getAttr(selRibbon + '.orientationX'), cmds.getAttr(selRibbon + '.orientationY'), cmds.getAttr(selRibbon + '.orientationZ'))
                endorient = (
                 cmds.getAttr(selRibbon + '.endorientX'), cmds.getAttr(selRibbon + '.endorientY'), cmds.getAttr(selRibbon + '.endorientZ'))
                ...
                if cmds.objExists(surface + '_skinCluster'):
                    cmds.skinCluster((surface + 'Shape'), e=1, ub=1)
                if cmds.objExists(surface + '_follicles'):
                    cmds.delete(surface + '_follicles')
                if cmds.objExists(surface + '_ctrljnts'):
                    cmds.delete(surface + '_ctrljnts')
                if cmds.objExists(surface + '_controls'):
                    cmds.delete(surface + '_controls')
    ...
    for i in range(1, ribbonSpans):
        if i == spanNumber:
            if i < 10:
                ...
                if not matrixRibbon:
                    cmds.parent((jointName.replace('joint', 'ctrljnt')), w=1)
    ...
    for i in range(1, ribbonDiv + 1):
        if not i == jointNumber:
            if i == ribbonDiv:
                ...
                if genRibbon:
                    cmds.move((moveAmount[0]), (moveAmount[1]), (moveAmount[2]), (ribbonNme + '_control' + buffer + '.cv[0:11]'), r=1, os=1)
            cntrlJointList.append(ribbonNme + '_ctrljnt' + buffer)

    if not genRibbon:
        if tweakCtrl:
            addTweakControls(ribbonNme, cntrlJointList, quadLimbInput, controlOri)
    else:
        if angleCtrl:
            addAngleControls(ribbonNme, cntrlJointList, quadLimbInput, controlOri)
        cmds.skinCluster(ribbonNme, cmds.ls((ribbonNme + '_ctrljnt_*', ribbonNme + '_angle_*'), type='joint'), tsb=1, dr=10, n=(ribbonNme + '_skinCluster'))
        if jntList and snapToInput:
            if quadLimbInput:
                limbJntCount = 4
            else:
                limbJntCount = 3
...
def addTweakControls(ribbonNme, cntrlJointList, quadLimbInput, controlOri, *args):
    ...
    if cmds.checkBox('jointOrientFlip', q=1, v=1):
        controlOri = (
         controlOri[0] * -1, controlOri[1] * -1, controlOri[2] * -1)
    elif quadLimbInput:
        tweakCount = 3
    else:
        tweakCount = 2
...
def custRibbon(*args):
    ...
    if cmds.objExists(ribbonNme):
        cmds.error('A ribbon with that name already exists, choose another name.')
    if jntRoot:
    ...
    if addControls:
        buildCustomControls(ribbonNme, jntRoot, 0, matrixRibbon, baseDivisions, quadLimbInput)
...
def antcgiStoreRibbonUI(storeValue, *args):
    ...
    for antcgiUICtrl in antcgiUICtrlList:
        controlTypeList = [
         'checkBox', 'intField', 'optionMenu', 'textField', 'floatField']
...
def antcgiRibbonUI():
    ...
    if cmds.window('antcgiRibbonTool', ex=1):
        cmds.deleteUI('antcgiRibbonTool')
    else:
    ...
    cmds.formLayout(mainLayout, edit=True, attachForm=[
        (newsFrame, 'left', 5), (newsFrame, 'top', 5), (newsFrame, 'right', 5),
      ...
      attachPosition=[
        ...
        (contGroupLabel, 'left', 0, 53),
        (linkText, 'left', 0, 30)])
    cmds.formLayout(newsLayout, edit=True, attachForm=[
        (antcgiRibbonNews, 'top', 5),
    ...
    elif cmds.objExists('tempinnercurve'):
        if cmds.objExists('tempoutercurve'):
            cmds.button('polyRibbonButton', e=1, en=1)
