...
def autoFillUI(armleg, doWhat, *args):
    if not cmds.optionVar(ex=('antcgiIKFK_fk_sh_' + armleg + '_ctrl')):
        ...
        if armleg == 'leg':
            ...
            radius = 'tibia'
            wrist = 'ankle'
        cmds.textField('fk_shoulder_ctrl_input', e=1,
                       tx=(shoulder + '_FK_ctrl'))
...
def storeLoad(armleg, doWhat, *args):
    if doWhat == 'store':
        cmds.optionVar(sv=('antcgiIKFK_fk_sh_' + armleg + '_ctrl',
                           cmds.textField('fk_shoulder_ctrl_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_fk_el_' + armleg + '_ctrl',
                           cmds.textField('fk_elbow_ctrl_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_fk_wr_' + armleg + '_ctrl',
                           cmds.textField('fk_wrist_ctrl_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_fk_sh_' + armleg + '_jnt',
                           cmds.textField('fk_shoulder_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_fk_el_' + armleg + '_jnt',
                           cmds.textField('fk_elbow_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_fk_wr_' + armleg + '_jnt',
                           cmds.textField('fk_wrist_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_ik_sh_' + armleg + '_jnt',
                           cmds.textField('ik_shoulder_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_ik_el_' + armleg + '_jnt',
                           cmds.textField('ik_elbow_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_ik_wr_' + armleg + '_jnt',
                           cmds.textField('ik_wrist_jnt_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_ik_el_' + armleg + '_ctrl',
                           cmds.textField('ik_elbow_ctrl_input', q=1, tx=1)))
        cmds.optionVar(sv=('antcgiIKFK_ik_wr_' + armleg + '_ctrl',
                           cmds.textField('ik_wrist_ctrl_input', q=1, tx=1)))
    else:
        cmds.textField('fk_shoulder_ctrl_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_sh_' + armleg + '_ctrl')))
        cmds.textField('fk_elbow_ctrl_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_el_' + armleg + '_ctrl')))
        cmds.textField('fk_wrist_ctrl_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_wr_' + armleg + '_ctrl')))
        cmds.textField('fk_shoulder_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_sh_' + armleg + '_jnt')))
        cmds.textField('fk_elbow_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_el_' + armleg + '_jnt')))
        cmds.textField('fk_wrist_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_fk_wr_' + armleg + '_jnt')))
        cmds.textField('ik_shoulder_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_ik_sh_' + armleg + '_jnt')))
        cmds.textField('ik_elbow_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_ik_el_' + armleg + '_jnt')))
        cmds.textField('ik_wrist_jnt_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_ik_wr_' + armleg + '_jnt')))
        cmds.textField('ik_elbow_ctrl_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_ik_el_' + armleg + '_ctrl')))
        cmds.textField('ik_wrist_ctrl_input', e=1, tx=cmds.optionVar(
            q=('antcgiIKFK_ik_wr_' + armleg + '_ctrl')))
...
def matchIKFK(whichWay, *args):
    ...
    found = 0
    nameCheckList = [
        fk_shoulder_ctrl, fk_elbow_ctrl, fk_wrist_ctrl, ik_shoulder, ik_elbow, ik_wrist,
    ...
    if found == 11:
        if cmds.objExists(fk_elbow_locator):
            ...
            cmds.error(
                'No reference locator detected, please create one at the bind pose.')
    else:
        cmds.error(
            "Some names didn't match, please check the script editor for details.")


def antcgiIKFKMatchUI():
    if cmds.window('IKFKMatch', ex=1):
        cmds.deleteUI('IKFKMatch')
    window = cmds.window(
        'IKFKMatch', t='IK<>FK Matching Tool v1.5', w=275, h=400, mnb=0, mxb=0, s=1)
    mainLayout = cmds.formLayout(numberOfDivisions=100)
    mainText = cmds.text(
        'mainText', label='IK<>FK Matching Tool by @antCGi', align='left', h=20)
    ...
    helpText = cmds.text(
        l='<a href="https://www.youtube.com/watch?v=7R_0omGY-Ms/?q=HTML+link">[?]</a>', hl=True)
    fk_ctrls_label = cmds.text(
        'fk_ctrls_label', label='FK Controls', align='left', h=20, fn='boldLabelFont')
    fk_jnts_label = cmds.text(
        'fk_jnts_label', label='FK Joints', align='left', h=20, fn='boldLabelFont')
    ik_ctrls_label = cmds.text(
        'ik_ctrls_label', label='IK Controls', align='left', h=20, fn='boldLabelFont')
    ik_jnts_label = cmds.text(
        'ik_jnts_label', label='IK Joints', align='left', h=20, fn='boldLabelFont')
    fk_shoulder_ctrl_label = cmds.text(
        'fk_shoulder_ctrl_label', label='Shoulder', align='left', h=20, w=60)
    fk_shoulder_ctrl_input = cmds.textField(
        'fk_shoulder_ctrl_input', w=120, h=20, ann='Shoulder Control Name.')
    fk_elbow_ctrl_label = cmds.text(
        'fk_elbow_ctrl_label', label='Elbow', align='left', h=20, w=60)
    fk_elbow_ctrl_input = cmds.textField(
        'fk_elbow_ctrl_input', w=120, h=20, ann='Elbow Control Name.')
    fk_wrist_ctrl_label = cmds.text(
        'fk_wrist_ctrl_label', label='Wrist', align='left', h=20, w=60)
    fk_wrist_ctrl_input = cmds.textField(
        'fk_wrist_ctrl_input', w=120, h=20, ann='Wrist Control Name.')
    fk_shoulder_jnt_input = cmds.textField(
        'fk_shoulder_jnt_input', w=120, h=20, ann='Shoulder Joint Name.')
    fk_elbow_jnt_input = cmds.textField(
        'fk_elbow_jnt_input', w=120, h=20, ann='Elbow Joint Name.')
    fk_wrist_jnt_input = cmds.textField(
        'fk_wrist_jnt_input', w=120, h=20, ann='Wrist Joint Name.')
    ik_elbow_ctrl_input = cmds.textField(
        'ik_elbow_ctrl_input', w=120, h=20, ann='Elbow Joint Name.')
    ik_wrist_ctrl_input = cmds.textField(
        'ik_wrist_ctrl_input', w=120, h=20, ann='Wrist Joint Name.')
    ik_shoulder_ctrl_label = cmds.text(
        'ik_shoulder_ctrl_label', label='Shoulder', align='left', h=20, w=60)
    ik_shoulder_jnt_input = cmds.textField(
        'ik_shoulder_jnt_input', w=120, h=20, ann='Shoulder Joint Name.')
    ik_elbow_ctrl_label = cmds.text(
        'ik_elbow_ctrl_label', label='Elbow', align='left', h=20, w=60)
    ik_elbow_jnt_input = cmds.textField(
        'ik_elbow_jnt_input', w=120, h=20, ann='Elbow Joint Name.')
    ik_wrist_ctrl_label = cmds.text(
        'ik_wrist_ctrl_label', label='Wrist', align='left', h=20, w=60)
    ik_wrist_jnt_input = cmds.textField(
        'ik_wrist_jnt_input', w=120, h=20, ann='Wrist Joint Name.')
    matchSideLabel = cmds.text(
        'matchSideLabel', label='Side', align='left', h=20)
    matchSideMenu = cmds.optionMenu('matchSideMenu', h=20, ann='Side.')
    cmds.menuItem(l='Left')
    cmds.menuItem(l='Right')
    button01 = cmds.button(
        w=80, h=25, l='[IK > FK]', p=mainLayout, c=(partial(matchIKFK, 'IKFK')))
    button02 = cmds.button(
        w=80, h=25, l='[FK > IK]', p=mainLayout, c=(partial(matchIKFK, 'FKIK')))
    button03 = cmds.button(
        w=100, h=25, l='[Add Reference Locator]', p=mainLayout, c=addRefLoc)
    storeNamesLabel = cmds.text(
        'storeNamesLabel', label='Store Names', align='left', h=20)
    button04 = cmds.button(w=60, h=20, l='[ ARM ]', p=mainLayout, c=(
        partial(autoFillUI, 'arm', 'store')), ann='Store the arm names.')
    button05 = cmds.button(w=60, h=20, l='[ LEG ]', p=mainLayout, c=(
        partial(autoFillUI, 'leg', 'store')), ann='Store the leg names.')
    loadNamesLabel = cmds.text(
        'loadNamesLabel', label='Load Names', align='left', h=20)
    button06 = cmds.button(w=60, h=20, l='[ ARM ]', p=mainLayout, c=(
        partial(autoFillUI, 'arm', 'load')), ann='Load the arm names.')
    button07 = cmds.button(w=60, h=20, l='[ LEG ]', p=mainLayout, c=(
        partial(autoFillUI, 'leg', 'load')), ann='Load the leg names.')
