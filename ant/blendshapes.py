...
def antcgiConnectBS_UI():
    if cmds.window('BSRigTool', ex=1):
        cmds.deleteUI('BSRigTool')
    window = cmds.window(
        'BSRigTool', t='Blendshape Rigging Tool v1.5', w=325, h=310, mnb=0, mxb=0, s=0)
    mainLayout = cmds.formLayout(numberOfDivisions=100)
    mainText = cmds.text(
        'mainText', label='Blendshape Rigging Tool by @antCGi', align='left', h=20)
    ...
    helpText = cmds.text(
        l='<a href="https://www.youtube.com/watch?v=QhkT_U5S9x8/?q=HTML+link">[?]</a>', hl=True)
    bsNodeLabel = cmds.text(
        'limbLabel', label='BlendShape Node', align='left', h=20)
    bsYLabel = cmds.text(
        'bsYLabel', label='Y Driven BlendShape', align='left', h=20)
    bsXLabel = cmds.text(
        'bsXLabel', label='X Driven BlendShape', align='left', h=20)
    bsZLabel = cmds.text(
        'bsZLabel', label='Z Driven BlendShape', align='left', h=20)
    connectXBox = cmds.checkBox(
        'connectXBox', l='Connect X', align='left', h=20, v=1)
    connectYBox = cmds.checkBox(
        'connectYBox', l='Connect Y', align='left', h=20, v=1)
    connectZBox = cmds.checkBox(
        'connectZBox', l='Connect Z', align='left', h=20, v=1)
    multiplierLabel = cmds.text(
        'multiplierLabel', label='Scale Multiplier', align='left', h=20)
    multiplierInput = cmds.floatField(
        'multiplierInput', h=20, ann='Adjust control size.', v=100)
    button01 = cmds.button(
        w=150, h=25, l='[GO]', p=mainLayout, c=antcgiConnectBS)
    ...
    bsNode = cmds.optionMenu(
        'bsNode', h=20, ann='Primary Blend Shape Node?', cc=updateBSAttr)
    ...
    bsAttr_Y = cmds.optionMenu(
        'bsAttr_Y', h=20, ann='Which shape is driven by the Y axis?')
    bsAttr_X = cmds.optionMenu(
        'bsAttr_X', h=20, ann='Which shape is driven by the X axis?')
    bsAttr_Z = cmds.optionMenu(
        'bsAttr_Z', h=20, ann='Which shape is driven by the Z axis?')
...
def antcgiConnectBS(*args):
    ...
    for connect in connectCheck:
        if 'bsMulti' in connect:
            cmds.warning('Control already connected.')

    if attrCheckList:
        for attrCheck in attrCheckList:
            if attrCheck == 'offsetOptions':
                offsetOptions = 1
            if attrCheck == 'X_Multi':
                X_Multi = 1
            if attrCheck == 'Y_Multi':
                Y_Multi = 1
            if attrCheck == 'Z_Multi':
                Z_Multi = 1

    if not offsetOptions:
        cmds.addAttr(controlName, ln='offsetOptions',
                     nn='-----', at='enum', en='OFFSET')
    ...
    cmds.shadingNode('multiplyDivide', au=1, n=(
        controlName.lower() + '_bsMulti'))
    if doY:
        if not Y_Multi:
            cmds.connectAttr((controlName + '.ty'),
                             (controlName.lower() + '_bsMulti.input1Y'), f=1)
            cmds.connectAttr((controlName.lower() + '_bsMulti.outputY'),
                             (bsNodeName + '.' + bsAttrName_Y), f=1)
            cmds.addAttr(controlName, ln='Y_Multi',
                         at='double', dv=multiAmount)
            cmds.setAttr((controlName + '.Y_Multi'), e=1, keyable=1)
            cmds.connectAttr((controlName + '.Y_Multi'),
                             (controlName.lower() + '_bsMulti.input2Y'), f=1)
    if doX:
        if not X_Multi:
            cmds.connectAttr((controlName + '.tx'),
                             (controlName.lower() + '_bsMulti.input1X'), f=1)
            cmds.connectAttr((controlName.lower() + '_bsMulti.outputX'),
                             (bsNodeName + '.' + bsAttrName_X), f=1)
            cmds.addAttr(controlName, ln='X_Multi',
                         at='double', dv=multiAmount)
            cmds.setAttr((controlName + '.X_Multi'), e=1, keyable=1)
            cmds.connectAttr((controlName + '.X_Multi'),
                             (controlName.lower() + '_bsMulti.input2X'), f=1)
    if doZ:
        if not Z_Multi:
            cmds.connectAttr((controlName + '.tz'),
                             (controlName.lower() + '_bsMulti.input1Z'), f=1)
            cmds.connectAttr((controlName.lower() + '_bsMulti.outputZ'),
                             (bsNodeName + '.' + bsAttrName_Z), f=1)
            cmds.addAttr(controlName, ln='Z_Multi',
                         at='double', dv=multiAmount)
            cmds.setAttr((controlName + '.Z_Multi'), e=1, keyable=1)
            cmds.connectAttr((controlName + '.Z_Multi'),
                             (controlName.lower() + '_bsMulti.input2Z'), f=1)
