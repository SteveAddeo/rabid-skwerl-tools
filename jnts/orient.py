...
def base_joint(joint, jnt_list, to_world=True, chain_to_world=False):
    if to_world:
        orient_joint(joint, jnt_list[1])
    if chain_to_world:
        orient_joint(joint, jnt_list[1], local=True)
    else:
        orient_joint(joint, jnt_list[1], jnt_list[2])


def mid_joints(joint, i, jnt_list, to_world=False):
    # Center Chains align to World Up
    if to_world:
        orient_joint(joint, jnt_list[i + 1])
    else:
        orient_joint(joint, jnt_list[i + 1], jnt_list[i - 1])
...
def joints_in_chain(joints=None, orient_tip=True, group=None, base_to_world=True, chain_to_world=False):
    if joints is None:
    ...
    for i, jnt in enumerate(joints):
        ...
        if i == 0:
            base_joint(jnt, jnt_list=joints, to_world=base_to_world, chain_to_world=chain_to_world)
        # Set orientation for the Tip Joint
        elif i == len(joints) - 1:
            tip_joint(jnt, joints[i - 1], to_joint=orient_tip)
        # Set orientation for all joints in between
        else:
            mid_joints(jnt, i, jnt_list=joints, to_world=chain_to_world)
